...
import { Upload, CheckCircle, X, AlertCircle, Loader2, ArrowLeft, Image as ImageIcon } from 'lucide-react';
import PageTransition from '../components/ui/PageTransition';
import SectionTitle from '../components/ui/SectionTitle';

// --- Interfaces ---
interface CustomCategory { id: string; name: string; description: string; image: string; }
interface FormData { name: string; email: string; phone: string; category: string; description: string; budget: string; dimensions: string; woodType: string; timeframe: string; }
interface ApiResponse { success: boolean; message: string; customOrder?: { _id: string; }; }

// --- Data ---
const customCategories: CustomCategory[] = [ { id: 'Tables', name: 'Tables', description: 'Custom dining tables, coffee tables, side tables...', image: 'https://images.pexels.com/photos/3935349/pexels-photo-3935349.jpeg?auto=compress&cs=tinysrgb&w=600' }, { id: 'Chairs', name: 'Chairs & Seating', description: 'Ergonomic chairs, dining chairs, sofas...', image: 'https://images.pexels.com/photos/1669799/pexels-photo-1669799.jpeg?auto=compress&cs=tinysrgb&w=600' }, { id: 'Storage', name: 'Storage Solutions', description: 'Cabinets, bookshelves, sideboards...', image: 'https://images.pexels.com/photos/2079249/pexels-photo-2079249.jpeg?auto=compress&cs=tinysrgb&w=600' }, { id: 'Beds', name: 'Beds & Bedroom', description: 'Bed frames, nightstands, dressers...', image: 'https://images.pexels.com/photos/1743229/pexels-photo-1743229.jpeg?auto=compress&cs=tinysrgb&w=600' }, { id: 'Other', name: 'Other Furniture', description: 'Let us know what you have in mind!', image: 'https://images.pexels.com/photos/6267516/pexels-photo-6267516.jpeg?auto=compress&cs=tinysrgb&w=600' } ];
const woodOptions: string[] = ['Teak', 'Sheesham', 'Mango Wood', 'Oak', 'Walnut', 'Maple', 'Pine', 'Other', 'Not Sure'];
const timeframeOptions: string[] = ['1-2 months', '3-4 months', '5-6 months', 'Flexible', 'Urgent', 'Not Specified'];
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api/v1';

// --- Component ---
const CustomOrderPage: React.FC = () => {
    const initialFormData: FormData = { name: '', email: '', phone: '', category: '', description: '', budget: '', dimensions: '', woodType: '', timeframe: '', };
    ...
    // --- Handlers ---
    const handleCategorySelect = (category: CustomCategory) => { setFormData((prev) => ({ ...prev, category: category.name })); setFormStep(2); window.scrollTo(0, 0); };
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => { const { name, value } = e.target; setFormData((prev) => ({ ...prev, [name]: value })); if (formStatus === 'error' || formStatus === 'success') { setFormStatus('idle'); setSubmitMessage(''); } };
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => { /* ... file handling logic ... */ if (e.target.files) { const newFiles = Array.from(e.target.files); const validFiles = newFiles.filter(file => ['image/jpeg', 'image/png', 'image/webp', 'application/pdf'].includes(file.type) && file.size <= 5 * 1024 * 1024); const combinedFiles = [...uploadedFiles, ...validFiles].slice(0, 5); setUploadedFiles(combinedFiles); if (validFiles.length !== newFiles.length) { setSubmitMessage("Some files were not added (invalid type or >5MB). Max 5 files."); setFormStatus('error'); } else { setSubmitMessage(''); setFormStatus('idle'); } } if (fileInputRef.current) fileInputRef.current.value = ""; };
    const removeFile = (indexToRemove: number) => { setUploadedFiles(prevFiles => prevFiles.filter((_, index) => index !== indexToRemove)); };
    const handlePrevStep = () => { setFormStep((prev) => Math.max(1, prev - 1)); window.scrollTo(0, 0); };
    const handleNextStep = () => { setFormStep((prev) => Math.min(3, prev + 1)); window.scrollTo(0, 0); };

    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setFormStatus('submitting'); setSubmitMessage('');
        const submissionData = new FormData();
        Object.entries(formData).forEach(([key, value]) => submissionData.append(key, value));
        uploadedFiles.forEach((file) => submissionData.append('referenceImages', file, file.name));
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/custom-order/new`, {
                ...
                // credentials: 'include', // Uncomment if route requires authentication
            });
            ...
            setSubmitMessage(result.message || 'Request submitted successfully!');
            ...
            setSubmitMessage(error.message || 'An error occurred submitting your request.');
        }
         // Loading state is handled implicitly by formStatus !== 'submitting'
    };

    const handleSendAnother = () => { setFormStatus('idle'); setSubmitMessage(''); setFormStep(1); window.scrollTo(0, 0); }
                ...
                <div className="container-custom max-w-4xl">
                    ...
                    {formStatus === 'success' ? (
                         <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-green-50 border border-green-300 rounded-lg shadow-lg p-8 text-center">
                             ...
                             <button onClick={handleSendAnother} className="mt-6 btn-outline text-sm">Submit Another Request</button>
                        </motion.div>
                    ) : (
                            ...
                            {/* Form Content */}
                             <AnimatePresence mode="wait">
                                <motion.form
                                    ...
                                    exit={{ opacity: 0, x: formStep < 3 ? -20 : 20 }}
                                ...
                                >
                                    ...
                                    {formStep === 1 && (
                                         <div>
                                            <h3 className="text-xl font-poppins font-semibold mb-6 text-center text-neutral-dark">1. What type of furniture?</h3>
                                    ...
                                    {formStep === 2 && (
                                         <div className="max-w-3xl mx-auto">
                                            <h3 className="text-xl font-poppins font-semibold mb-8 text-center text-neutral-dark">2. Tell Us About Your Idea</h3>
                                            <div className="space-y-5">
                                                 <div><label htmlFor="description" className="form-label">Description*</label><textarea id="description" name="description" value={formData.description} onChange={handleInputChange} placeholder="Describe the piece, style, features, materials, finish, etc." className="input-field h-32 resize-none" required></textarea></div>
                                                 ...
                                                 <div>
                                                     ...
                                                     {uploadedFiles.length > 0 && ( <div className="mb-3 mt-1 space-y-2"> <ul className="text-sm space-y-1"> {uploadedFiles.map((file, index) => ( <li key={index} className="flex items-center justify-between text-gray-700 bg-gray-100 px-3 py-1.5 rounded border border-gray-200"> <span className="truncate pr-2 flex items-center gap-1.5"><ImageIcon size={14}/> {file.name} ({ (file.size / 1024).toFixed(1) } KB)</span> <button type="button" onClick={() => removeFile(index)} className="text-red-500 hover:text-red-700 ml-2 focus:outline-none flex-shrink-0 p-0.5 rounded hover:bg-red-100" aria-label={`Remove ${file.name}`}> <X size={16}/> </button> </li> ))} </ul> </div> )}
                                                     {uploadedFiles.length < 5 && ( <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:border-primary transition-colors cursor-pointer bg-white hover:bg-gray-50" onClick={() => fileInputRef.current?.click()}> <div className="space-y-1 text-center"> <Upload size={36} className="mx-auto text-gray-400" /> <div className="flex text-sm text-gray-600 justify-center"> <span className="font-medium text-primary hover:text-primary-dark cursor-pointer">Click to upload</span> <input id="file-upload" name="referenceImages" type="file" className="sr-only" onChange={handleFileChange} multiple accept="image/jpeg,image/png,image/webp,application/pdf" ref={fileInputRef} disabled={uploadedFiles.length >= 5}/> <p className="pl-1 hidden sm:block">or drag and drop</p> </div> <p className="text-xs text-gray-500">PNG, JPG, PDF up to 5MB. Max files: {5 - uploadedFiles.length}</p> </div> </div> )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="space-y-5">
                                                ...
                                                <div className="pt-2">
                                                    <label className="flex items-start cursor-pointer">
                                                         ...
                                                         <span className="text-xs text-gray-600">I agree to the processing of my data as per the <Link id="privacy-policy-link" to="/privacy-policy" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Privacy Policy</Link> and consent to be contacted regarding my request.</span>
                                                    </label>
                                                </div>
                                            </div>
...
const formLabelClass = "block text-sm font-medium text-gray-700 mb-1.5";
